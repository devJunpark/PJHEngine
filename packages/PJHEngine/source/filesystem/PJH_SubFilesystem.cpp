...
PJH_SubFilesystem::FileIteratorPair 
PJH_SubFilesystem::getFileList() const { return FileIteratorPair{ _filechilds.begin(), _filechilds.end() }; }

PJH_SubFilesystem::DirIteratorPair
PJH_SubFilesystem::getDirList() const { return DirIteratorPair{ _dirchilds.begin(), _dirchilds.end() }; }

PJH_SubFilesystem::FileIteratorPair
PJH_SubFilesystem::getFileList()
{
	cachingFileList();
	return FileIteratorPair{ _filechilds.begin(), _filechilds.end() };
}

PJH_SubFilesystem::DirIteratorPair
PJH_SubFilesystem::getDirList() 
{
	cachingDirList();
	return DirIteratorPair{ _dirchilds.begin(), _dirchilds.end() }; 
}


//The second version :: remove all previous caching and do not search previous caching.
void PJH_SubFilesystem::cachingFileList()
{
	namespace fs = boost::filesystem;
	...
	//The frist version :: do not remove previous caching but will search previous caching every iter.
	fs::directory_iterator iter{ rootpath };
	fs::directory_iterator enditer;
	for (iter; iter != enditer;++iter)
	{
		bool isReguFile = fs::is_regular_file(*iter);
		bool isFile = !fs::is_directory(*iter);
...
void PJH_SubFilesystem::cachingDirList()
{
